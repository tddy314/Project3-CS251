...
async function getTotalOwed(user) {
	//console.log(user);
	try {
	let totalValue = await BlockchainSplitwise.getTotalOwe(user);
	return totalValue;
	}
	catch(error) {
...
async function getLastActive(user) {
	let timeS = await BlockchainSplitwise.getLastActive(user);
...
async function add_IOU(creditor, amount) {
	...
	//const accountOfsender = await getSignerByAddress(sender);
    await BlockchainSplitwise.add_IOU(creditor, amount, path, ok, sender);

	//const money = await getTotalOwed(sender);
	//console.log(money);
}
...
//const money = await getTotalOwed("0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266");